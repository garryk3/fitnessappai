...
class ProfileController {
  ...
  /// Подгружает следующую порцию замеров.
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      measurements.value = page;
      hasMore.value = page.length < total;
      latest.value = page.isEmpty ? null : page.first;
      await _refreshChart();
    } finally {
  ...
  Future<void> _loadPage() async {
    final page = await measurementRepository.getPage(
      offset: measurements.value.length,
      limit: _pageSize,
    );
    measurements.value = [...measurements.value, ...page];
    final total = await measurementRepository.count();
    hasMore.value = measurements.value.length < total;
  }
