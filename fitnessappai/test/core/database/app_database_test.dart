...
void main() {
  ...
  test('app_meta: insert и round-trip', () async {
    await database.into(database.appMeta).insert(
          AppMetaCompanion.insert(key: 'dataVersion', value: const Value('2')),
        );
    final row = await (database.select(database.appMeta)
          ..where((t) => t.key.equals('dataVersion')))
        .getSingleOrNull();
  ...
  test('app_meta: delete', () async {
    await database.into(database.appMeta).insert(
          AppMetaCompanion.insert(key: 'temp', value: const Value('1')),
        );
    await (database.delete(database.appMeta)
          ..where((t) => t.key.equals('temp')))
        .go();
  ...
  test('duplicate key: upsert заменяет значение', () async {
    await database.into(database.appMeta).insert(
          AppMetaCompanion.insert(key: 'key', value: const Value('a')),
        );
    await database.into(database.appMeta).insert(
